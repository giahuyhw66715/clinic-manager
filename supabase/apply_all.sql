...
-- Fires on INSERT (booking) and on UPDATE of patient_id/date/time_slot (reschedule),
-- NOT on status-only changes (cancel, complete, no-show, ...).
...
-- (status -> 'delivered'). Applies no matter which UI path triggers delivery.
-- ----------------------------------------------------------------------------
...
-- Covers the edge case where delivery happens before the doctor finishes the
-- visit (invoice does not exist yet at delivery time).
